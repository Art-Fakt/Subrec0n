...
def massTakeOver(targets, threads):
	...
	if len(targets) <= 100000:
		print("{0} {1} {2}".format(colored("\n[+]-Scanning", "white"), colored(len(targets), "green"), colored("domains for potential takeover...", "white")))

	else:
		print("{0} {1} {2}".format(colored("\n[+]-Scanning", "white"), colored(len(targets), "green"), colored("domains for potential takeover, in chunks of 100,000...", "white")))
	...
	for targetChunk in targetChunks:
		with ThreadPoolExecutor(max_workers=threads) as executor:
			...
			try:
				...
				if numberOfChunks == 1:
					completed = tqdm(completed, total=len(targetChunk), desc="  --> {0}".format(colored("Progress", "cyan")), dynamic_ncols=True, leave=leaveFlag)

				else:
					completed = tqdm(completed, total=len(targetChunk), desc="  --> {0}".format(colored("Progress {0}/{1}".format(iteration, numberOfChunks), "cyan")), dynamic_ncols=True, leave=leaveFlag)
...
def init(db, domain, old_takeovers, threads):
	...
	print("    --> {0} {1}".format(colored("New takeover vulnerabilities that were identified:", "white"), colored(len(takeovers), "green")))

	for takeover in takeovers:
		print("      --> {0}: {1}, {2}".format(colored(takeover[0], "cyan"), colored(takeover[1], "yellow"), colored(takeover[2], "yellow")))
