...
def init(db, domain, markovState, markovLength, markovQuantity, hideWildcards, threads):
	...
	if len(baseList) <= 1000:
		print(Fore.CYAN + Style.BRIGHT + f"\n[+]-Performing markov based permutations on " + Fore.GREEN + str(len(baseList)) + Fore.CYAN + Style.BRIGHT + f" hostname-parts..." + Style.RESET_ALL)
	else:
		print(Fore.CYAN + Style.BRIGHT + f"\n[+]-Performing markov based permutations on " + Fore.GREEN + str(len(baseList)) + Fore.CYAN + Style.BRIGHT + f" hostname-parts in chunks of 1,000..." + Style.RESET_ALL)
	...
	for baseChunk in baseChunks:
		...
		with ThreadPoolExecutor(max_workers=1) as executor:
			...
			print("{0} {1}".format(colored("\n[+]-Generating markov candidates for chunk", "white"), colored(str(iteration) + "/" + str(numberOfChunks), "green")))
			try:
				...
				if numberOfChunks == 1:
					completed = tqdm(completed, total=len(baseChunk), desc="  --> {0}".format(colored("Progress", "cyan")), dynamic_ncols=True, leave=leaveFlag)
		...
		print(Fore.CYAN + Style.BRIGHT + "\n[+]-Generated " + Fore.GREEN + str(len(finalMarkovified)) + Fore.CYAN + Style.BRIGHT + " markov candidates for chunk " + Fore.GREEN + str(iteration - 1) + "/" + str(numberOfChunks) + Style.RESET_ALL)
